...
def customer_history(id):
    ...
    return render_template(
        "customer_history.html", customer=customer, orders=orders, invoices=invoices, total_spent=total_spent
    )
...
def reports():
            ...
            sales_data.append(
                {
                    "date": d.isoformat(),
                    "orders": len(subset),
                    "revenue": sum(o.get("total_amount", 0) for o in subset),
                }
            )
    ...
    order_status = [{"status": key, "count": value} for key, value in status_counts.items()]
    ...
    for order in selected_orders:
        ...
        if cid not in customer_stats:
            customer_stats[cid] = {
                "name": customers.get(cid, {}).get("name", "Unknown"),
                "orders": 0,
                "spent": 0,
            }
